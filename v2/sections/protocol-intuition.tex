...
\rogeri{a rough explanation about how double-spending (i.e. double vote-count) is prevented should be included somewhere in this section (as it is explained how coercion and vote-buying is mitigated).}

\begin{figure}[h]	
	\centerline{\includegraphics[width=320pt,draft=false]{\figs/protocol-intuition-flow.pdf}}
	\setlength{\abovecaptionskip}{0pt} % Space between figure and caption
	\caption{Overview of the protocol flow. {\color{blue}Figure is outdated with respect to new actors and phases.}}
...
\begin{enumerate}
	...
	During the voting period, two actors interact continuously: \textit{voters}, who cast their encrypted ballots, and \emph{sequencers}, independent entities responsible for collecting, verifying, and processing these ballots before committing them to the shared state. Both processes, ballot submission by voters (\ref{sec:protocol-intuition:vote-casting}) and vote batching by sequencers (\ref{sec:protocol-intuition:vote-batching}), occur continuously and in parallel throughout this phase.
