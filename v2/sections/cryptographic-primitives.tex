...
\begin{itemize}
	...
	\item ${\tree.\hlset{\MTInsert}}()$: it inserts a new leaf in the tree. If the position is passed as an input, the element is inserted in that position. Otherwise, the new element is inserted in the first empty position. The root is updated.
	\item ${\tree.\hlset{\MTRemove}}()$: it removes a leaf from the tree. The root is updated.
	\item ${\tree.\hlset{\MTOpen}}()$: it returns the Merkle tree inclusion proof of the given leaf.
	...
	%	\item ${\MT.\UpdateRoot}_{\pp}({\leaf, \pos, \root})$: it updates the root - I am not sure this function is needed!
	\item ${\tree.\hlset{\MTVerify}}
	()$: it takes a Merkle proof and a root as an input and verifies the proof against the given root. The output is accept/reject.
