...
\begin{itemize}
	\item BW6-761 curve: $E_1/\F_p$ defined by equation $E_1: y^2 = x^3 - 1$, with subgroups $\G_1, \G_2$ of prime order $q$, and an efficiently computable pairing \(e:\G_1\times\G_2 \rightarrow \G_T,\) where $\G_T \subset \F_{q^{6}}$ and has order $q$ as well. 
    \item BLS12-377 curve: $E_2/\F_q$ defined by equation \(E_2: y^2 = x^3 + 1,\) with subgroups $\G_1, \G_2$ of prime order $r$, and an efficiently computable pairing \(e:\G_1\times\G_2 \rightarrow \G_T,\) where $\G_T \subset \F_{q^{12}}$ and has order $r$ as well. 
    \item BN254 curve: $E_3/\F_s$ defined by equation \( E_3: y^2 = x^3 + 3\), where with subgroups $\G_1, \G_2$ of prime order $t$, and an efficiently computable pairing \(e:\G_1\times\G_2 \rightarrow \G_T,\) where $\G_T \subset \F_{q^{12}}$ and has order $t$ as well. 
\end{itemize}

\paragraph{Finite fields.}
\begin{itemize}
	\item $\F_p$: base field of the BW6-761 curve.
	\item $\F_q$: scalar field of $\G_1, \G_2, \G_T$ and base field of the BLS12-377 curve.
	\item $\F_r$: scalar field of XXX.
	\item $\F_s$: scalar field of XXX.	
	\item $\F_t$: scalar field of XXX.
\end{itemize}
