...
\martai{The output of the hash does not match, CHECK!}
...
The \davinci protocol uses sparse Merkle trees (SMTs) as defined in ~\cite{baylina19sparse}. Essentially, in this kind of trees each leaf corresponds to a key-value pair, with empty leaves explicitly represented to enable both inclusion and non-inclusion proofs. The key feature of these trees is that the hash of each entry defines a unique traversal path in the tree from root to leaf, regardless of insertion order. When inserting, the algorithm descends until it encounters an empty node or a leaf; if there is a hash prefix collision, the tree is split at the first differing bit, creating new intermediate nodes appropriately. SMTs support efficient membership and non‑membership proofs, enabling a party to prove that a given key-value is (or is not) present in the tree without revealing the content. 

In \davinci, both the census (voter registry with weights) and the vote state are maintained using SMTs. The root hashes are committed on-chain, while full tree contents remain off-chain. Sequencers generate and submit Merkle proofs to the verification circuits, which reconstruct and validate the root transitions. This design allows compact, verifiable state updates and enables independent verification without relying on trusted data sources.

\noi To each Merkle tree $\tree$, we associate the following functions:

\begin{itemize}
%	\item ${\tree.\hlset{\MTGenerate}}(data)$: given a set of data, instantiate the tree.	
	\item ${\tree.\hlset{\MTRoot}}()$: it returns the root of the tree.
	\item ${\tree.\hlset{\MTInsert}}(path, \leaf)$: efectivament, el path se segueix per it inserts a new leaf in the tree.
	\item ${\tree.\hlset{\MTUpdate}}(\leaf[old], \leaf)$: given an old leaf in the tree, it updates the leaf content to \leaf.
%	\item ${\tree.\hlset{\MTRemove}}()$: it removes a leaf from the tree. The root is updated.
	\item ${\tree.\hlset{\MTMembershipProof}}(\leaf)$: it returns the Merkle tree inclusion proof of the given \leaf.
	\item ${\tree.\hlset{\MTNonMembershipProof}}(\leaf)$: it returns the Merkle tree non-inclusion proof of the given \leaf.
	%	\item ${\MT.\UpdateRoot}_{\pp}({\leaf, \pos, \root})$: it updates the root.
	\item ${\tree.\hlset{\MTVerify}}(\leaf, proof, root)$: it takes a Merkle proof and a root as an input and verifies the proof of the leaf against the given root. 
\end{itemize}
...
\martai{Add MT.Update, for those ciphertexts that are overwrites.}

\martai{Clarify leaf -- if hash of the leaf or address as path.}

\subsection{Zero-knowledge proof systems}
\label{sec:cryptographic-primitives:zkp}

Groth16~\cite{groth2016size} with different curves. 
