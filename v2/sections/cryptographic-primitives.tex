...
\begin{itemize}
	...
	\item ${\tree.\hlset{\MTInsert}}()$: it inserts a new leaf in the tree. The root is updated.
%	\item ${\tree.\hlset{\MTRemove}}()$: it removes a leaf from the tree. The root is updated.
	\item ${\tree.\hlset{\MTProof}}()$: it returns the Merkle tree inclusion proof of the given leaf.
	%	\item ${\MT.\UpdateRoot}_{\pp}({\leaf, \pos, \root})$: it updates the root.
	\item ${\tree.\hlset{\MTVerify}}
	()$: it takes a Merkle proof and a root as an input and verifies the proof against the given root. 
\end{itemize}
