...
\begin{itemize}
	...
	\item $\tree.\hlset{\MTVerify}(\leaf, \MTProof, \root{})$: it verifies that the given proof corresponds to the claimed root and leaf.
\end{itemize}
...
{\color{blue} Description of KZG~\cite{kate2010constant} with SRS \textbf{[add link to Ethereum's BLS12-381 SRS]}, $\hlset{\CCommit}()$, and $\hlset{\CVerify}()$.}\\
...
{\color{blue}{\textbf{TODO}: add \hlset{\SPubKey} that extracts $\pk$ from $\signature$ (in Ethereum it's called \texttt{ECRecover}).}}\\
...
To ensure that no single entity controls the decryption key, DAVINCI employs a \ac{DKG} protocol to jointly derive the ElGamal encryption key pair used for ballots encryption~\cite{todocitaton}. Participants who contribute to the \ac{DKG} ceremony are called \emph{\keywardens}, and they are each identified by a unique index~$i$. The outcome is a collective public key (\epk) that is published on-chain and used by voters to encrypt their ballots, while the corresponding private key is secret-shared among the \keywardens. Only a threshold number $t$ out of $n$ \keywardens can later collaborate to decrypt the tally. Unlike classical \ac{DKG} protocols where shares are exchanged in the clear, our construction uses encrypted shares and \ac{ZK-SNARK} proofs of correctness. This allows the Ethereum smart contract to verify compliance without learning any of the underlying secrets, ensuring both security and verifiability in a fully decentralized setting. 
...
This approach ensures that the encryption public key is securely generated in a decentralized way, that each \keywarden learns only their own secret share, and that the correctness of the entire \ac{DKG} procedure is verifiable on-chain. Misbehavior, such as submitting invalid shares, can be detected and penalized through the slashing mechanism enforced by the smart contract (see Section~\ref{sec:token}).
