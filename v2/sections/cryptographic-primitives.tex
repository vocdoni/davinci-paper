...
\Davinci uses tow hash functions. On the one side, we use Keccak256~\cite{bertoni2011keccak} for Ethereum address derivation and Poseidon~\cite{grassi2021poseidon} for everywhere else. 
...
This approach ensures that the encryption public key is securely generated in a decentralized way, that each \keywarden learns only their own secret share, and that the correctness of the entire \ac{DKG} procedure is verifiable on-chain. Misbehavior, such as submitting invalid shares, can be detected and penalized through the slashing mechanisms that may be enforced by the smart contract.% (see Section~\ref{sec:token}).
