...
%The rest of the paper is organized as follows. Section~\ref{sec:background} introduces the necessary background. Section~\ref{sec:protocol-intuition} provides a high-level overview of a voting process, omitting technical details for clarity. Section~\ref{sec:cryptographic-primitives} presents the cryptographic primitives used in the protocol, together with the concrete instantiations adopted in our design. Section~\ref{sec:vocdoni-protocol} describes the full voting process, while Section~\ref{sec:ballot-protocol} specifies the ballot protocol, whose rules are enforced by the process management smart contract. Section~\ref{sec:token} discusses the role of the Vocdoni token within the incentive structure of the system. Section~\ref{sec:analysis} analyzes the protocol, covering its security properties, implementation details, and performance evaluation. Finally, Section~\ref{sec:conclusions} concludes the paper with final remarks.
...
\begin{itemize}
	...
	\item Section~\ref{sec:cryptographic-primitives}: cryptographic primitives used in the voting process. A description of the primitives but also the specific protocols being used and the details of their instantiation.
