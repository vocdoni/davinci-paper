...
\begin{itemize}
	...
	\item Any address: vote \addresses: are stored within the State tree pointing to a voter's encrypted ballot. Addresses are used as indexes (i.e. path to the leaf that stores encrypted ballots).
	\item \textbf{voteIDs??}
\end{itemize}
...
\organizer
\begin{enumerate}
	\item Prepares the census data \censusdata and generates census Merkle tree.
	\item Generates the census commitment: $\censusroot = \hlget{\MTRoot}()$.
...
\begin{itemize}
	...
	\item Send the following data, which includes the encrypted vote, census membership proof, signature and vote proof, to the selected sequencer:
		\[ \vote = [ \processid, \voteid, \encryptedballot, \censuspf, \voteproof, \signature ]. \]
\end{itemize}

\martai{\\
	1. Add as a footnote that the address is not send because it can be recomputed from the signature.\\
	2. In the code, they call it \textit{vote}, \textit{verifierVote}. Alternative: \textit{envelope}.}
